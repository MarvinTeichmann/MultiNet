...
flags.DEFINE_string('resname',
                    None,
                    'Name to save the result.')

output = "/home/mifs/mttt2/cvfs/output/supp_material2"
...
def run_eval(load_out, output_folder, data_file):
    ...
    for image_file, output in gen:
        ...
        im_name = os.path.basename(image_file)
        im_name = "{}_{}_.png".format(im_name.split('.')[0], FLAGS.resname)
        new_im_file = os.path.join(output_folder, im_name)
...
def main(_):
    ...
    if FLAGS.resname is None:
        logging.error('Please provide a resname')
        exit(1)

    output_folder = os.path.join(output, FLAGS.resname)
